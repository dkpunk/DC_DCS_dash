...
#echo "\nip".$ip;
#echo "\nport".$port;
$filename=$ip.'_'.$port.'_DC.out';
echo "\nfilename".$filename;
$outfile=fopen($filename,'w');
chdir($s_path);
chdir('..');
$output=shell_exec('sh /var/www/html/SO/SO/ToolOpsDashBoard/RETINS/MQ_scripts/DCS_UN_DASH/Main_DC.sh '.$ip.' '.$port);
...
fclose($filename);
chdir($s_path);
$output1=shell_exec('sh format_output_DC.sh '.$filename);
echo "<p>$output1<p>";
?>
